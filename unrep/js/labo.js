...
function vueDefaut(ms){const v=plantes[cle].userData.vue;S.ctrl.minDistance=v.min;S.ctrl.maxDistance=v.max;volerVers(v.pos,v.cible,ms)}
...
// ----- textes et boutons -----
const LIB_COUPE={ginkgo:'🔍 Loupe : nervures',narcisse:'✂️ Couper le bulbe',lavande:'🔍 Loupe : tige et fleur'};
const GUIDE={
...
function majFiche(){const P=PLANTES[cle];$('fNom').textContent=P.nom;$('fLat').textContent=P.latin;$('fFam').textContent=P.famille;$('fVue').textContent='Vue 3D : '+P.vue.toLowerCase();$('piege').innerHTML='⚠️ <b>Piège d\'examen :</b> '+P.piege;
 ...
 $('bCoupe').textContent=LIB_COUPE[cle];$('bCoupe').classList.toggle('on',coupe);
...
function choisir(k){plantes[cle].visible=false;cle=k;const G=plantes[k];G.visible=true;
 coupe=(k==='narcisse'&&MODE==='saisons');if(G.userData.setCoupe)G.userData.setCoupe(coupe);
 S.ombre.visible=(k!=='narcisse');
 construirePastilles();majFiche();vueDefaut(900);
 if(MODE==='saisons')choisirSaison(saison,true);
}
function choisirSaison(s,direct){saison=s;transition=direct?1:0;const [t,x]=PLANTES[cle].saisons[s];
 $('dTitre').textContent=t;$('dTexte').textContent=x;$('dTexte').className='';$('dNum').textContent=NOMS_SAISONS[s].toUpperCase();
 document.querySelectorAll('.s').forEach(b=>b.classList.toggle('on',b.dataset.s===s));
 $('friseI').style.width=((ORDRE_SAISONS.indexOf(s)+1)/4*100)+'%';
 neige(s==='hiver');
 if(direct)plantes[cle].userData.saison(s,1);
}
$('bCoupe').onclick=()=>{const G=plantes[cle];coupe=!coupe;$('bCoupe').classList.toggle('on',coupe);
 if(cle==='narcisse'){G.userData.setCoupe(coupe);if(coupe)volerVers(new THREE.Vector3(.2,-.35,2.1),new THREE.Vector3(0,-.55,0),900)}
 else if(cle==='lavande'){G.userData.setCoupe(coupe);if(coupe)volerVers(new THREE.Vector3(1.75,.95,1.75),new THREE.Vector3(.95,.62,.15),900);else vueDefaut()}
 else{const f=G.userData.cible.feuille;f.updateWorldMatrix(true,false);const p=f.localToWorld(new THREE.Vector3(0,.5,0));if(coupe){const n=new THREE.Vector3(0,0,1).applyQuaternion(f.getWorldQuaternion(new THREE.Quaternion()));volerVers(p.clone().add(n.multiplyScalar(.7)),p,900)}else vueDefaut()}
};
...
function majChute(dt){
 const actif=MODE==='saisons'&&cle==='ginkgo'&&saison==='automne';
...
function boucle(now){requestAnimationFrame(boucle);const dt=Math.min(.05,(now-tPrec)/1000);tPrec=now;
 ...
 const n=plantes.narcisse.userData;if(n.bourg){const on=MODE==='saisons'&&saison==='ete';n.bourg.material.emissive.setRGB(on?.7+.3*Math.sin(now/260):0,on?.55+.25*Math.sin(now/260):0,on?.05:0)}
 majChute(dt);
