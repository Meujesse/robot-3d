// Camille « vivante » (moteur repris de Lisa) : postures, caméra, respiration, clignement, bouche synchronisée.
...
(function () {
  ...
  const PLANS = { gros: { D: 0.19, y: 0.40 }, buste: { D: 0.125, y: 0.33 }, americain: { D: 0.082, y: 0.26 }, pied: null };
  ...
  const LARGE = { salut: ['americain'], explique: ['americain', 'buste'], bravo: ['americain'], doute: ['americain', 'buste'], telephone: ['americain'],
    reflechit: ['buste', 'gros'], ecoute: ['buste', 'gros', 'americain'] };
  const ECOUTE = ['ecoute'], REFLEXION = ['reflechit'], PARLE = ['explique'];

  function create({ stage, onPose, base, depart }) {
    const BASE = base || 'camille/';
    ...
    function charge(nom) {
      const c = CFG[nom];
      if (!IMG[nom]) {
        IMG[nom] = new Image(); IMG[nom].src = BASE + nom + (c.fixe ? '.jpg' : '.webp');
    ...
    function cadre(nomPlan, c, off) {
      ...
      let k, tx, ty;
      if (nomPlan === 'plein') { k = 1.13 * Math.max(W / c.w, H / c.h); tx = Math.min(-8, Math.max(W - c.w * k + 8, 0.30 * W - ex * k)); ty = Math.min(-8, Math.max(H - c.h * k + 8, 0.37 * H - ey * k)); }
      else if (nomPlan === 'pied') { k = 0.96 * H / c.h; tx = W / 2 - (c.w / 2) * k + off * W; ty = 0.02 * H; }
    ...
    function choisitPlan(nom, force) {
      const c = CFG[nom]; if (c.fixe) return 'plein'; const dispo = LARGE[nom] || PLANS_PAR_TYPE[c.plan];
    ...
    const api = {
      async init() {
        ...
        await setPose(depart || new URLSearchParams(location.search).get('pose') || 'salut', { plan: 'americain' });
  ...
  window.CamilleRig = { create, syllabes };
})();
